...
const authController = {
  register: (req, res) => {
    try {
      ...
      authModel
        .checkEmailRegistered(email.toLowerCase()) // call model to check email exist or not
        .then((result) => {
          bcrypt.hash(password, 10, (err, hash) => {
            // encrypt password before insert to db
            if (err) {
              ...
              authModel
                .register(data) // calling model to register data
                .then((result) => {
                  success(result, null, "success", "Register user berhasil"); // output if success
                })
                .catch((err) => {
                  failed(result, err.message, "failed", "Register user gagal"); // output if failed
                });
            }
          });
        })
  ...
  login: (req, res) => {
    try {
      ...
      authModel
        .login(email) // call model and send email to check email exist or not
        .then((results) => {
          if (results.rowCount > 0) {
            // condition if email exist
            bcrypt
              .compare(password, results.rows[0].password) // compare plain password with hash password on db
              .then((match) => {
                if (match) {
                  const token = jwtToken(results.rows[0]);
                  successWithToken(
                    res,
                    token,
                    results.rows[0].id,
                    "success",
                    "login sukses"
                  ); // output if bcrypt compare return true
                } else {
